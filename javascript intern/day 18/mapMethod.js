...
const result = marks.map((m) =>
  m > 40
    ? `you are passed with ${m - 40} margin`
    : `you have failed with ${40 - m} marks`
);
...
const feedback = marks.map((m, i) => {
  return m > 40
    ? `student number ${i + 1} with marks ${m} has passed with ${m - 40} margin`
    : `student number ${i + 1} with marks ${m} has failed with ${
        40 - m
      } margin`;
});
